...
def main():
    st.set_page_config(
        page_title="Kannada Voice → English Speech",
        page_icon="🎤")
    ...
    if st.button(
        ...
        try:
            ...
            if "format not recognised" in str(e).lower():
                st.info(
                    "💡 **Tip**: Convert your Kannada MP3 to WAV format online first")
            elif "speaker_wav" in str(e).lower():
                st.info(
                    "💡 **Tip**: Make sure your audio file has clear speech (5+ seconds)")
            else:
